...
Dans cette section, nous présentons des concepts et outils qui pourraient s'avérer utile pour l'analyse de texte (Text Mining) 

### Différence entre **character** et **factor**

Quand on fait du traitement sur des chaînes de caractères, il est certainement préférable d'utiliser le type **character** plutôt que le type **factor**. En effet, si la classe **factor** peut s'avérer fort utile pour l'analyse statistique, celle-ci a aussi des spécificités qui peuvent s'avérer contraignantes pour l'analyse de texte. 

A vrai dire, la classe **factor** a probablement été définie par des statisticiens, car elle répond parfaitement à la définition d'une variable qualitative. Autrement dit, il s'agit de caractéristiques non numériques, observées sur des individus, dont les valeurs possibles sont connues à l'avance et dénombrables. Par exemple, à la question avez-vous aimé ce film, on pourra répondre `pas du tout`, `un peu`, `moyennement`, `beaucoup`, `énormément` et seules ces valeurs sont autorisées.

Supposons qu'on récupère des données sur des notes de film dont un extrait serait contenu dans le vecteur suivant :

```{r}
harry_potter_6 <- c("bcp", "moyennement", "moyen", "non", "beacup")
```

La première chose serait de corriger les erreurs qui peuvent être dûes, dans la plupart des cas, à des erreurs de saisie. Ce serait le rôle de l'analyste de texte de programmer un certain nombre de fonctions afin de corriger ces erreurs. Une fois ces erreurs corrigées, on obtientrait ainsi le vecteur suivant : 

```{r}
harry_potter_6 <- c("beaucoup", "moyennement", "moyennement", "pas du tout", "beaucoup")
```

A partir de là, il pourrait être judicieux de transformer ce vecteur en **factor** de la façon suivante : 

```{r}
harry_potter_6_f <- factor(harry_potter_6)
```

Les facteurs ont des modalités pré-définies qui sont retournées avec la fonction *levels()*. 

```{r}
levels(harry_potter_6_f)
```

Dans l'instruction suivante, on souhaite ajouter la valeur `énormément`, mais comme celle-ci n'a pas été spécifiée au départ, elle n'est pas acceptée, ce qui génère ici un message d'avertissement et une valeur manquante

```{r}
harry_potter_6_f[6] <- "énormément"
harry_potter_6_f[6]
```

C'est pourquoi, il est important de spécifier toutes les valeurs possibles au moment de la création du **factor** (même si certaines valeurs ne sont pas encore incluses dans le vecteur) avec l'option `levels =`.

```{r}
harry_potter_6_f <- factor(harry_potter_6, 
  levels = c("pas du tout", "un peu", "moyennement", "beaucoup", "énormément")
)
```
...
Un autre avantage du **factor** est que lorsqu'on va faire des tableaux de contingence ou des graphiques, l'ordre des modalités sera identique et dans le même ordre que celui défini initialement. Par exemple : 

```{r}
table(harry_potter_6_f)
```

ce qui n'est bien entendu pas le cas pour les vecteurs de caractères, où l'ordre alphabétique est systématiquement utilisé :

```{r}
table(harry_potter_6)
```

En fait, lorsqu'on tape la commande suivante : 

```{r}
str(harry_potter_6_f)
```

on constate qu'un **factor** peut être considéré comme un vecteur de valeurs entières où chaque entier correspond à une étiquette. Autrement dit, une autre façon de créer un **factor** serait de définir un vecteur d'entiers, de préciser avec **levels** quels sont les entiers qui sont acceptés (ici 1, 2, 3, 4, 5) et d'associer à chacune de ces valeurs une étiquette ou **label**. Par exemple : 

```{r}
harry_poter_6 <- c(4, 3, 3, 1, 4)
harry_poter_6 <- factor(harry_poter_6, levels = 1:5, 
    labels = c("pas du tout", "un peu", "moyennement", "beaucoup", "énormément"))
```

**Remarque :** si on oublie d'associer un **level** à un **label**, cela a pour conséquence de créer une valeur manquante. 

Dans le cas de vecteurs de taille importantes, le stockage d'un **factor** est un peu moins volumineux qu'un objet **character**, car au lieu de stocker des chaînes de caractères, on stocke un vecteur de valeurs entières. Toutefois, sur des objets de petite taille, ce n'est pas le cas puisqu'un **factor** contient aussi l'information contenant la correspondance entre les valeurs entières et les étiquettes.  

Ici, on utilise la fonction *object.size()* qui indique la taille allouée à un objet en mémoire vive : 

```{r}
object.size(harry_potter_6)
object.size(harry_potter_6_f)
```


**Remarque** : pendant longtemps, la plupart des fonctions qui importent des jeux de données (comme `read.table()`) retournaient les variables non numériques sous forme de **factor**. Cet aspect a été critiqué par certains programmeurs dont Hadley Wickam (nous en verrons les raisons un peu plus tard), ce qui explique pourquoi aujourd'hui les variables non numériques sont codées sous forme de **character**. 

L'idée serait donc d'utiliser la classe **character** pour le traitement de texte et une fois que les différentes modalités des variables qualitatives ont été identifiées, convertir le vecteur **factor** pour faciliter l'analyse statistique. 
...
### Transformer une variable quantitative en character 
...
* **Q1. Codage et comptage :** donner un équivalent de l'enchaînement des fonctions *cut()* et *table()* utilisées précédemment.



### Fonctions de base pour l'analyse de texte
